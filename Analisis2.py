...
plt.plot(df['Year'], df['%_Naturales'], color='green', linewidth=4, label='Espacios Naturales', marker='o')
plt.plot(df['Year'], df['%_Antropogenicos'], color='red', linewidth=4, label='Usos Humanos', marker='s')
...
# Añadir valores de inicio y fin en el gráfico
plt.annotate(f'{df["%_Naturales"].iloc[0]:.1f}%',
             xy=(df['Year'].iloc[0], df['%_Naturales'].iloc[0]),
             xytext=(10, 10), textcoords='offset points', color='green', fontweight='bold')
plt.annotate(f'{df["%_Naturales"].iloc[-1]:.1f}%',
             xy=(df['Year'].iloc[-1], df['%_Naturales'].iloc[-1]),
